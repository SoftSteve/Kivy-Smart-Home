...
from .custom_blind_buttons import CustomBlindButton  # Assuming CustomBlindButton is in a separate file
from .more_options import CustomBlindButton2  # Assuming CustomBlindButton2 is in a separate file
from custom_ui.customgradient import CustomGradient  # Assuming CustomGradient is in a separate file


class BlindsControlPanel(Widget):
    def __init__(self, store, **kwargs):
        ...
        self.blind_left = CustomBlindButton(
        ...
        self.blind_right = CustomBlindButton(
        ...
        self.grid_layout.add_widget(self.blind_left)
        self.grid_layout.add_widget(self.blind_right)
        ...
        self.t_half_btn = CustomBlindButton2(
            text='Lower 50%',
            button_id='t50%',
            state=False,
            store = self.store,
        ...
        self.t_threequarter_btn = CustomBlindButton2(
            text='Lower 75%',
            button_id='t75%',
            state=False,
            store = self.store,
        ...
        self.t_schedule_btn = CustomBlindButton2(
            text='Schedule',
            button_id='tscd',
            state=False,
            store = self.store,
        ...
        self.t_schedule_btn.bind(switch_active=self.open_time_picker)

        self.t_lower = CustomBlindButton2(
            text='All Blinds',
            button_id='t100%',
            state=False,
            store = self.store,
        ...
        self.top_btn_container.add_widget(self.t_lower)
        self.top_btn_container.add_widget(self.t_schedule_btn)
        self.top_btn_container.add_widget(self.t_half_btn)
        self.top_btn_container.add_widget(self.t_threequarter_btn)
    ...
    def refresh_ui(self, *args):
        ...
        for device in self.store.get_all_devices():
            ...
            blinds_pos = device["blinds_start"]
            if button_id not in self.dont_add:
                self.new_button = CustomBlindButton(
                    text=name,
                    id=button_id,
                    state=saved_state,
                    blinds_start = blinds_pos,
                    store=self.store,
                )
                ...
                self.store.update_states(button_id, name, saved_state, blinds_pos)   
...
class BlindsControlScreen(FloatLayout):
    def __init__(self, store, **kwargs):
        super().__init__(**kwargs)
        self.store = store

        self.control_panel = BlindsControlPanel(
            size_hint=(.75, 1),
