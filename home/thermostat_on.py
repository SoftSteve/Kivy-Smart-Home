...
class ThermostatOnScreen(FloatLayout):
        ...
        def __init__(self, therm_store, **kwargs):
            ...
            self.thermostat = ThermostatWidget(size_hint=(1, .8),pos_hint={'center_x':.5, 'center_y':.57}, callback = self.update_label, therm_store=self.therm_store)
        ...
        def update_label(self, *args):
                self.mode_label.font_size = self.width * .05
                self.thermostat.temp_label.font_size = self.width * 0.2

        def update_buttons(self, *args):
            for button in [self.heat, self.cool, self.eco, self.fan]:
                button.width = self.width * 0.15
                button.height = self.height * 0.18
                button.font_size = self.width * 0.1
                button.halign = 'center'

            for buttons in [self.btn_decrease, self.btn_increase]:
                buttons.width = self.width * 0.125
                buttons.height = self.height * 0.15
                buttons.font_size = self.width * 0.07

        def update_spacing(self, *args):
            self.button_container2.spacing = self.width * .07
            self.button_container.spacing = self.width * 0.04